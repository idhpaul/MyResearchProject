 ...
 public:
 		...
 		m_ssl_stream(m_ios, m_ssl_context)
 	{
 		...
 		m_ssl_stream.set_verify_callback(
 			boost::bind(&SyncSSLClient::on_peer_verify, this, _1, _2));

 		std::cout << "sslclient set_verify_callback ok" << std::endl;
 	...
 	void connect() {
 		...
 		std::cout << "sslclient connect ok" << std::endl;


 		// Perform the SSL handshake.
 		m_ssl_stream.handshake(asio::ssl::stream_base::client);

 		std::cout << "sslclient handshake ok" << std::endl;
 ...
 int main()
 {
 	...
 	try {
 		...
 		std::cout << "sslclient class ok" << std::endl;
